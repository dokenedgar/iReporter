...
var interventionVal = function () {
    function interventionVal() {
        _classCallCheck(this, interventionVal);
    }

    _createClass(interventionVal, [{
        key: 'create',
        ...
        key: 'fetchSpecificRecord',
        value: function fetchSpecificRecord(req, res, next) {
            req.check('id').isLength({
                min: 6
            }).withMessage('Please provide an intervention id with at atleast 6 characters').isLength({
                max: 10
            }).withMessage('Please provide an intervention id with at most 10 characters').isNumeric().withMessage('You provided a non-integer in the intervention id field');
            if (req.validationErrors()) {
                return res.status(400).json({
                    status: 400,
                    error: req.validationErrors()
        ...
        value: function editLocation(req, res, next) {
            req.check('id').isLength({
                min: 6
            }).withMessage('Please provide a intervention id with at atleast 6 characters').isLength({
                max: 10
            }).withMessage('Please provide an intervention id with at most 10 characters').isNumeric().withMessage('You provided a non-integer in the intervention id field');
            req.check('userid').isLength({
                min: 6
    ...
    return interventionVal;
}();

var interventionValidation = exports.interventionValidation = new interventionVal();